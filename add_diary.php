...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$form['game_id'] || !in_array($form['status'], ['playing', 'completed', 'backlog'], true) || $form['playtime_hours'] === '' || !$form['last_played_at']) {
        ...
        // Prevent duplicate diary entries for the same game by the same user
        $existing = $pdo->prepare('SELECT id FROM diary_entries WHERE user_id = ? AND game_id = ? LIMIT 1');
        $existing->execute([$user['id'], (int)$form['game_id']]);
        if ($existing->fetch()) {
            $error = 'You already have a diary entry for this game. You can edit it from your diary.';
        } else {
            $stmt = $pdo->prepare('INSERT INTO diary_entries (user_id, game_id, status, playtime_hours, liked, last_played_at, achievement_notes) VALUES (?, ?, ?, ?, ?, ?, ?)');
            $stmt->execute([
                $user['id'],
                (int)$form['game_id'],
                $form['status'],
            ...
            flash('success', 'Diary entry added successfully.');
        ...
        <div class="col-xl-8">
            <div class="glass-card p-4 p-lg-5">
                <div class="d-flex justify-content-between align-items-center mb-4 gap-3 flex-wrap">
                    <div>
                        <h1 class="fw-bold mb-1">Add Diary Entry</h1>
                        <p class="text-muted-custom mb-0">Track your progress, playtime, and notes for a game.</p>
                    </div>
                ...
                <form method="POST">
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="achievement_notes" class="form-control" rows="6" placeholder="Write your achievement progress, quest notes, or gameplay highlights here..." required><?= e($form['achievement_notes']) ?></textarea>
                        </div>
                    </div>
                    <div class="mt-4 action-row">
                        <button class="btn btn-warning">Save Diary Entry</button>
